...
export function DateSelect3({ month, day, year, onChange }: DateSelect3Props) {
  ...
  return (
    <div className="ll-date-select">
      ...
      <select value={day} onChange={(e) => onChange({ month, day: Number(e.target.value), year })}>
        {days.map((d) => (
      ...
      <select value={year} onChange={(e) => onChange({ month, day, year: Number(e.target.value) })}>
        {years.map((y) => (
